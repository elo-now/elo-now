...
pub fn configure_client(client: &mut ClientApp) -> Result<(), String> {
    ...
    if let Some(url) = option_env!("TAURI_ELO_WAKE_URL") {
        client
...
mod mobile {
    ...
    pub(super) fn messages_read(
        ...
        let Some(url) = option_env!("TAURI_ELO_WAKE_URL") else {
            return Ok(());
        };
            ...
            && (op == "maintain" || op == "enable")
        {
            ...
            if !saved.token.is_empty() && saved.next_attempt <= time() {
                ...
                let result = request(
                    saved,
                    "",
                    Some(json!({"token":saved.token,"notify_key":saved.route.notify_key})),
                    reqwest::Method::POST,
                )
                .await?;
