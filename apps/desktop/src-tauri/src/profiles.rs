...
pub(super) fn select(app: &tauri::AppHandle, path: &Path) -> Result<()> {
    let name = path
        .file_name()
        .and_then(|s| s.to_str())
        .ok_or("Invalid profile selection")?;
...
pub(super) fn registration_path(app: &tauri::AppHandle) -> Result<PathBuf> {
    let current = active(app)?;
    if current.try_exists()? {
        // Registration beside a saved profile uses its own destination, just
        // like recovery. Selection changes only after initialization succeeds.
        new_profile(app)
    } else {
        Ok(current)
    }
...
async fn recover_without_backup(
    draft: &ProfileDraft,
    path: PathBuf,
    ...
    client.begin_space_setup().await?;
    Ok(client)
}
...
fn qr(code: &str) -> Result<qrcode::QrCode> {
    if code.len() > 4096 {
        return Err("This code is too large".into());
    }
    // Hosted device links include an identity-bound access proof. Keep all
    // signed bytes, using the standard lower-redundancy QR level when needed.
    Ok(qrcode::QrCode::new(code).or_else(|error| match error {
        qrcode::types::QrError::DataTooLong => {
            qrcode::QrCode::with_error_correction_level(code, qrcode::EcLevel::L)
        }
        other => Err(other),
    })?)
}
...
async fn run(app: &tauri::AppHandle, state: tauri::State<'_, State>, v: Value) -> Result<Value> {
    ...
    match op {
        "account_deletion_status" => {
            ...
            let saved: Value = serde_json::from_slice(&vault::read_private(&path)?)?;
            if saved["local_complete"] != true {
                return Ok(json!({"status":"pending"}));
            }
            let receipts: Vec<elo_core::app::account_deletion::Receipt> =
        ...
        "recover" => {
            ...
            if (!name.is_empty() || state.backup.is_none()) && !record::valid_display_name(name) {
            ...
            let mut client = if let Some(bytes) = &state.backup {
                ...
                recover_without_backup(draft, path.clone(), password, name).await?
            };
            ...
            if state.backup.is_some() && !name.is_empty() {
                client
                    .operate(json!({"op":"set_profile_name","name":name}))
                    .await?;
            }
            ...
            app.state::<crate::background_history::BackgroundHistory>()
                .resume();
            #[cfg(desktop)]
            crate::desktop_activity::update(app, &view);
            return Ok(view);
        }
        "backup" => {
            let client = state.client.as_ref().ok_or("The profile is locked")?;
        ...
        "account_deletion_check" => {
            let client = state.client.as_ref().ok_or("The profile is locked")?;
            if text(&v, "identity")? != client.identity_id().to_string() {
                return Err("The selected profile changed.".into());
            }
            return Ok(serde_json::to_value(
                client
                    .delete_account(
                        env!("ELO_CONFIGURED_SPACE_HOST"),
                        env!("ELO_CONFIGURED_WAKE"),
                        false,
                    )
                    .await?,
            )?);
        ...
        "remove" | "delete_account" => {
            ...
            let paths = if op == "delete_account" {
                saved(app)?
                    .iter()
                    .filter(|entry| entry["identity"] == v["identity"])
                    .map(|entry| {
                        Ok(base(app)?.join(entry["id"].as_str().ok_or("Invalid profile entry")?))
                    })
            ...
            if op == "delete_account" {
                let outcome = client
                    ...
                    .await?;
                if outcome.status != elo_core::app::account_deletion::Status::Pending {
                ...
                vault::write_private(
                    &receipts_path,
                    &serde_json::to_vec(&json!({"receipts":receipts,"local_complete":false}))?,
            ...
            #[cfg(mobile)]
            {
                ...
                // recovered copies of the same identity with a different password.
                let identity = v["identity"].as_str().ok_or("Invalid profile identity")?;
            ...
            if op == "delete_account" {
                let path = base(app)?.join("account-deletion-receipts.json");
                let mut receipt: Value = serde_json::from_slice(&vault::read_private(&path)?)?;
                receipt["local_complete"] = json!(true);
                vault::write_private(&path, &serde_json::to_vec(&receipt)?, true)?;
            }
        }
        _ => {
...
mod tests {
    ...
    fn hosted_device_codes_fit_both_qr_exports_without_truncation() {
        ...
        assert!(qrcode::QrCode::new(&code).is_err());
        assert!(svg(&code).unwrap().contains("<svg"));
        assert!(qr_png(&code).unwrap().starts_with(b"\x89PNG\r\n\x1a\n"));
        assert!(svg(&"a".repeat(4097)).is_err());
    }
    #[tokio::test]
    async fn words_only_recovery_keeps_space_setup_after_reopen() {
        let temp = tempfile::tempdir().unwrap();
        let original = super::ProfileDraft::new().unwrap();
        let card = original.card();
        let draft =
            super::ProfileDraft::recover(&card.phrase, &card.identity_id.to_string()).unwrap();
        let path = temp.path().join("recovered");
        let password: super::SecretString = "synthetic recovered profile password".into();
        let client = super::recover_without_backup(
            &draft,
            path.clone(),
            password.clone(),
            "Recovered profile",
        ...
        assert_eq!(client.identity_id(), card.identity_id);
        assert_eq!(view["name"], "Recovered profile");
        assert_eq!(view["space_setup"], true);
        assert_eq!(view["spaces"], serde_json::json!([]));
        assert_eq!(view["streams"], serde_json::json!([]));
        assert!(!client.allows_default_space().unwrap());
        client.close().await.unwrap();

        let mut reopened = super::ClientApp::open(path, password, false).await.unwrap();
        reopened.enable_spaces().await.unwrap();
        let view = reopened.view().await.unwrap();
        assert_eq!(view["space_setup"], true);
        assert_eq!(view["spaces"], serde_json::json!([]));
        assert!(!reopened.allows_default_space().unwrap());
    ...
    fn removal_preserves_other_profiles_and_refuses_unrelated_files_or_links() {
        ...
        std::fs::write(
            child.join("space-vault-cache.age"),
            b"synthetic encrypted cache",
        )
        .unwrap();
        std::fs::write(
            profile.join("blocked.age"),
            b"synthetic encrypted block list",
        )
        .unwrap();
