...
import { messageCreatedAt, senderName, type View } from "./model";
import {
  streamSwipe,
  swipeDirection,
  unreadStreamEntries,
...
export function MessageStream({
  ...
  return (
    <section
      className="message-stream content-pane"
      hidden={!active}
    ...
    >
      ...
      <div className="stream-viewport" ref={viewport}>
        ...
        >
          <PullToRefresh
            className="stream-list page-content"
            enabled={active && !lift}
            ...
            {entries.slice(0, limit).map((entry, index) => {
              ...
              return (
                <Fragment key={entry.key}>
                  {index > 0 &&
                    messageDayKey(messageCreatedAt(entry.row)) !==
                      messageDayKey(
                        messageCreatedAt(entries[index - 1].row),
                      ) && (
                      <h3 className="message-day">
                        <span>
                          {formatMessageDay(messageCreatedAt(entry.row))}
                        </span>
                      </h3>
                    )}
