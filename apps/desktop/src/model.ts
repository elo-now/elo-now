...
export type SpaceRoleRequest = {
  space_id: string;
  space_name: string;
  revision: number;
  request: {
    id: string;
    kind: "remove_owner" | "transfer_primary" | "remove_member";
    target: string;
    target_name: string;
    requester: string;
...
export type View = {
  blocked_users?: { identity: string; name: string }[];
  paged?: boolean;
  partial?: boolean;
  space_setup?: boolean;
  space_creation?: {
...
/** Attachment descriptors use the Space service's acceptance time. Older
 * attachment records do not contain chat logical-time fields, so use that
 * signed descriptor value for presentation ordering and display dates. */
export function messageLogicalTime(row: Stream["rows"][number]): number {
  const dated = row.body.created_at ? Date.parse(row.body.created_at) : 0;
  return (
    row.body.logical_time ??
    row.body.attachment?.created_at_ms ??
...
export function messageCreatedAt(
  ...
  if (row.body.created_at) return row.body.created_at;
  const timestamp =
    row.body.attachment?.created_at_ms ??
    (row.body.kind === "deleted" ? row.body.logical_time : undefined);
