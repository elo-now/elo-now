...
export function useMessageHistory(
  ...
  const seed: typeof state | undefined =
    view &&
    chat &&
    // Prepared pages precede the current safety preferences. Re-read them when
    // any block exists so cached reactions and replies cannot reappear.
    !view.blocked_users?.length &&
    preparedHistoryMatches(
      prepared,
      view.identity,
      view.active_space,
