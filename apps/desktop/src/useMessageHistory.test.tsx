...
function FirstPaint({
  ...
  const history = useMessageHistory(
    blocked
      ? { ...view, blocked_users: [{ identity: "other", name: "Blocked" }] }
      : view,
    chat,
    true,
    query,
...
test("cached push content is reverified against active blocks before rendering", () => {
  const html = renderToStaticMarkup(<FirstPaint blocked />);
  expect(html).toContain('data-ready="false"');
  expect(html).not.toContain("Ready message");
});
