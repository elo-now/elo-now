...
describe("user display preferences", () => {
  it("keeps a custom drawing across motif changes and reloads without changing colors or opacity", () => {
    const drawing: CustomMotif = {
      v: 1,
      smooth: false,
      strokes: [
        [
    ...
    try {
      ...
      savePreferences(selectMotif(readPreferences(), "elo"));
      expect(readPreferences().customMotif).toEqual(drawing);
      savePreferences(selectMotif(readPreferences(), "custom"));
      expect(readPreferences()).toEqual(prefs);
      stored = JSON.stringify({
        ...prefs,
        customMotif: { v: 1, smooth: true, strokes: [["<svg>"]] },
      });
      expect(readPreferences().motif).toBe("elo");
      expect(readPreferences().customMotif).toBeNull();
      expect(readPreferences().colorOverrides).toEqual(prefs.colorOverrides);
      expect(readPreferences().motifOpacity).toBe(0.08);
    } finally {
      vi.unstubAllGlobals();
    }
  });

  it("reports failed persistence so an unsaved drawing can stay open", () => {
    vi.stubGlobal("localStorage", {
      setItem: () => {
