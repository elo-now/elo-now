...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const content = toast && (
    <>
      <ToastLayer key={toast.id} data-leaving={leaving} data-tone={toast.tone}>
        {toast.open ? (
          <button
            type="button"
...
function ToastLayer({ children, ...props }: React.ComponentProps<"aside">) {
  ...
  return (
    <aside {...props} ref={ref} className="toast" popover="manual">
      {children}
    </aside>
  );
}
