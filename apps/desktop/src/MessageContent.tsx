      ...
      <div className="message-content">
        ...
        >
          ...
          <span className="message-time">
            <time
              dateTime={row.body.created_at}
              title={formatTimestamp(row.body.created_at)}
            >
              {formatMessageTime(row.body.created_at, showDate)}
            </time>
            {row.pinned && (
              <span
        ...
        {children}
        {chat && <MessageReactions chat={chat} row={row} />}
      </div>
    </>
  );
}
