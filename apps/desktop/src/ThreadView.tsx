...
export function ThreadView({
  ...
  return (
    <section className="thread-view" aria-label={t("thread.title")}>
      <ScreenHeader
        ...
        >
          ...
          {rows.map((row, index) => (
            <ThreadMessage
              ...
              index={index}
              seen={seen}
              root={row.id === thread.rootId}
              target={scrollTarget?.id === row.id}
...
function ThreadMessage({
  ...
  return (
    <>
      {index > 0 &&
        messageDayKey(row.body.created_at) !==
          messageDayKey(rows[index - 1].body.created_at) && (
          <h3 className="message-day">
            <span>{formatMessageDay(row.body.created_at)}</span>
          </h3>
        )}
