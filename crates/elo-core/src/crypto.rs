...
pub fn open_record(ciphertext: &[u8], identity: &age::x25519::Identity) -> Result<SignedRecord> {
    let bytes = open_bytes(ciphertext, identity, MAX_RECORD)?;
    SignedRecord::parse(&bytes).map_err(|_| CryptoError::Decrypt)
}
pub(crate) fn open_bytes(
    ...
    let decryptor = bounded_decryptor(ciphertext).map_err(|_| CryptoError::Decrypt)?;
...
pub fn open_object(ciphertext: &[u8], identity: &age::x25519::Identity) -> Result<SignedRecord> {
    let bytes = open_bytes(ciphertext, identity, crate::files::MAX_FILE_RECORD)?;
