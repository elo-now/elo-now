...
use crate::ids::{IdentityId, RecordId, SpaceId, StreamId};
...
impl MessageAction {
    pub fn target(&self) -> RecordId {
        match self {
            Self::Reaction { target, .. } | Self::Pin { target, .. } => *target,
        }
    }
    fn valid(&self) -> bool {
        match self {
...
impl ChatMessage {
    pub fn validate(&self) -> Result<()> {
        if self.v != 1 || !matches!(self.kind.as_str(), "chat.message" | "chat.action") {
            return Err(RecordError::Unsupported);
        }
        hex::<16>(&self.nonce)?;
        if !sorted_unique(&self.audience, 1, MAX_CHAT_MEMBERS)
            ...
            || match (&*self.kind, &self.payload.action) {
                ("chat.message", None) => !(1..=16384).contains(&self.payload.text.len()),
                ("chat.action", Some(action)) => {
