...
impl ProfileDraft {
    ...
    async fn save_with_name(
        ...
        let session = Session::recover(&self.card, self.card.identity_id)?;
        ...
        drop(session);
        let mut app =
            ClientApp::open_initialized(directory.clone(), password.clone(), false).await?;
        let initialized: Result<()> = async {
            if let Some(name) = name {
                app.set_profile_name(&name)?;
            }
            app.create_space(initial_channel, &self.card).await
        }
        .await;
        ...
        ClientApp::open(directory, password, false).await
    }
}
