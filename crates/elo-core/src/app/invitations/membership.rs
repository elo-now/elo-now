...
impl ClientApp {
    ...
    pub(super) async fn create_contact_chat(
        ...
        if !self.session.peers().iter().any(|p| p.write_token.is_some()) {
            return Err("Add a sync server before adding people.".into());
        }
    ...
    pub(super) async fn add_contact_members(
        ...
        let draft = state
            ...
            .clone();
        let current = &mut self.authorities.0[index];
    ...
    fn verified_membership(&self, packet: &Packet) -> Result<(Authority, shared::Offer, RecordId)> {
