...
impl ClientApp {
    ...
    pub(super) fn reply_inbox(&self, offer: &shared::Offer) -> Result<Option<Inbox>> {
        offer
            ...
            .as_ref()
            .map(|route| {
                Peer::new(route.descriptor(), self.allow_loopback)?;
    ...
    pub(super) fn queue_packet(
        ...
        Peer::new(target.descriptor(), self.allow_loopback)?;
    ...
    pub(in crate::app) async fn sync_invitations(&mut self, force: bool) -> Result<Value> {
        let team_changed = self.sync_team(force).await.unwrap_or(false);
        let time = now()?.as_millis() as u64;
    ...
    async fn poll_invitation_mailbox(
        ...
        let peer = Peer::new(mailbox.descriptor(), self.allow_loopback)?;
