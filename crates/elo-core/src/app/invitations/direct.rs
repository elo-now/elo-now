...
impl ClientApp {
    ...
    pub(super) async fn create_dm(&mut self, v: Value, mut state: Invitations) -> Result<Value> {
        ...
        if people.iter().any(|person| self.blocked.contains(*person)) {
    ...
    pub(super) async fn discover_offers(
        ...
        for (_, peer) in self
            ...
            .iter()
            .zip(&self.peers)
            .filter(|(descriptor, _)| descriptor.read_token.is_some())
        {
        ...
        for (key, peer, mut cursor) in peers.into_iter().take(2) {
            ...
            let result = self
                ...
                .await;
            let progressed = cursor.after != previous;
            ...
            if let Ok(more) = result {
                ...
                report.retry += 1;
                if progressed {
                    // Resume the committed batch before applying network backoff.
                    // A repeated failure at the same object still backs off.
                    report.more = true;
                    cursor.failures = 0;
                    cursor.next = 0;
                } else {
        ...
        for object in page.entries {
                ...
                && object.size_bytes <= (MAX_PACKET + 64 * 1024) as u64
            {
                ...
                if let Some(Packet::Invitation { bundle }) = packet
                    ...
                    && !self.blocked.contains(controller.identity())
                    && known
