...
impl ClientApp {
    pub(super) async fn open_contact(&mut self, v: Value, mut state: Invitations) -> Result<Value> {
        ...
        if self.blocked.contains(person) {
        ...
        let mut authority = self.authorities.0[index].clone();
        ...
        self.authorities.0[index] = authority.clone();
        if !authority
            .head()?
            .members
            .iter()
    ...
    pub(super) async fn receive_personal(
        ...
        if self.blocked.contains(authority.controller().identity()) {
            return Ok(false);
        }
