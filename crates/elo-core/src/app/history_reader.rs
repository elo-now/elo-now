...
pub(super) fn record_presentation_time(record: &SignedRecord) -> u64 {
    record.body()["logical_time"]
        .as_u64()
        .or_else(|| record.body()["attachment"]["created_at_ms"].as_u64())
        .unwrap_or(0)
}
...
impl HistoryView<'_> {
    ...
    pub(super) async fn open_sources(
        ...
        for source in sources {
            ...
            if matches!(
                r.body()["kind"].as_str(),
                Some("chat.message" | "chat.action" | "chat.locator")
            ) {
        ...
        rows.sort_by(|(left, _), (right, _)| {
            (
                record_presentation_time(left),
                left.body()["issuer_credential"].as_str().unwrap_or(""),
                left.id(),
            )
                .cmp(&(
                    record_presentation_time(right),
    ...
    pub(super) async fn history_page(&self, v: &Value) -> Result<Value> {
        ...
        let project = |(r, state): (SignedRecord, String)| {
            ...
            let message_id = if r.body()["kind"] == "chat.locator" {
                r.body()["locator"]["message_record_id"]
                    .as_str()
                    .unwrap_or(&id)
            } else {
                &id
