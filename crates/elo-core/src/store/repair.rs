...
impl ClientStore {
    ...
    pub(crate) async fn finish_scan_page(
        ...
        self.call(move |c| {
            ...
            if complete {
                tx.execute("UPDATE replica_copies SET missing=1 WHERE peer_id=?1 AND mailbox_id=?2 AND seen_round!=?3",
    ...
    pub(crate) async fn claim_repair(
        ...
        self.call(move |c| {
            ...
            let row:Option<(String,u64,String,i64,String)>=tx.query_row(
                "SELECT c.object_id,c.size_bytes,c.transfer_hint,c.attempts,s.storage_generation FROM replica_copies c JOIN replica_scans s USING(peer_id,mailbox_id) WHERE c.peer_id=?1 AND c.mailbox_id=?2 AND c.missing=1 AND c.next_attempt_local_ms<=?3 ORDER BY c.next_attempt_local_ms,c.object_id LIMIT 1",
    ...
    pub(crate) async fn copy_sources(
        ...
        self.call(move |c| {
            let mut q=c.prepare("SELECT peer_id,mailbox_id,size_bytes FROM replica_copies WHERE object_id=?1 AND size_bytes>0 ORDER BY missing,peer_id,mailbox_id LIMIT 128")?;
    ...
    pub(crate) async fn confirm_repair(
        ...
        self.call(move |c| {
            let tx=c.transaction_with_behavior(TransactionBehavior::Immediate)?;
            let changed=tx.execute("UPDATE replica_copies SET missing=0,receipt_record=?4 WHERE peer_id=?1 AND mailbox_id=?2 AND object_id=?3 AND missing=1 AND attempts=?5 AND EXISTS(SELECT 1 FROM replica_scans s WHERE s.peer_id=?1 AND s.mailbox_id=?2 AND s.storage_generation=?6)",
    ...
    pub async fn missing_copies(&self) -> Result<u64> {
        self.call(|c| {
            Ok(c.query_row(
                "SELECT count(*) FROM replica_copies WHERE missing=1",
                [],
                |r| read_count(r, 0),
            )?)
        })
