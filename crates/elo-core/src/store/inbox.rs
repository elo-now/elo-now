...
impl ClientStore {
        ...
        self.call(move |c| {
            ...
            let mut q = c.prepare(
                "SELECT r.record_id, s.object_id, s.source_index,
                 ...
                 WHERE r.space_id=?1 AND r.stream_id=?2 AND r.kind IN ('chat.message','file.shared') AND r.status IN ('LOCAL','ACCEPTED')
    ...
    pub(crate) async fn action_sources(
        ...
        self.call(move |c| {
            let mut q = c.prepare("SELECT r.record_id,s.object_id,s.source_index,r.status FROM records r JOIN record_sources s USING(record_id) WHERE r.space_id=?1 AND r.stream_id=?2 AND r.status IN ('LOCAL','ACCEPTED') AND ((?3 IS NULL AND r.kind='chat.action') OR (r.record_id=?3 AND r.kind IN ('chat.message','file.shared'))) ORDER BY r.record_id,s.source_index")?;
