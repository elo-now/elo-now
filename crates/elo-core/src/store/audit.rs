...
mod tests {
    ...
    async fn upgrade_preserves_old_history_and_does_not_invent_events() {
        ...
        old.execute_batch("DROP TABLE message_locators; ALTER TABLE replica_copies DROP COLUMN retention_expired; ALTER TABLE replica_copies DROP COLUMN pruned_record; DROP TABLE notification_outbox; DROP TABLE message_audit; DROP TABLE message_audit_meta; PRAGMA user_version=4; UPDATE outbox SET attempts=7,last_error_code='REMOTE_UNAVAILABLE'").unwrap();
        drop(old);
        let store = ClientStore::open(dir.path()).await.unwrap();
        assert_eq!(
