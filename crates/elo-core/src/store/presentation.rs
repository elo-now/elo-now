...
impl ClientStore {
    pub(crate) async fn message_sources_by_ids(
        ...
        self.call(move |c| {
            ...
            let mut q=c.prepare("SELECT record_id FROM records WHERE space_id=?1 AND stream_id=?2 AND kind IN ('chat.message','file.shared') AND status IN ('LOCAL','ACCEPTED') AND record_id IN (SELECT value FROM json_each(?3))")?;
    ...
    pub(crate) async fn message_sources_page(
        ...
        self.call(move |c| {
            let boundary = before.map(|id| c.query_row(
                "SELECT first_seen_local_ms FROM records WHERE space_id=?1 AND stream_id=?2 AND record_id=?3 AND kind IN ('chat.message','file.shared') AND status IN ('LOCAL','ACCEPTED')",
            ...
            let mut q = c.prepare(&format!("SELECT record_id FROM records WHERE space_id=?1 AND stream_id=?2 AND kind IN ('chat.message','file.shared') AND status IN ('LOCAL','ACCEPTED') AND (?3 IS NULL OR (first_seen_local_ms,record_id){comparison}(?3,?4)) ORDER BY first_seen_local_ms {direction},record_id {direction} LIMIT ?5"))?;
    ...
    pub(crate) async fn summary_sources(
        ...
        self.call(move |c| {
            ...
            let mut q=c.prepare("WITH recent AS (SELECT record_id,status FROM records WHERE space_id=?1 AND stream_id=?2 AND kind IN ('chat.message','file.shared') AND status IN ('LOCAL','ACCEPTED') ORDER BY first_seen_local_ms DESC,record_id DESC LIMIT 1000) SELECT record_id FROM recent WHERE record_id=(SELECT record_id FROM recent LIMIT 1) OR (status!='LOCAL' AND record_id NOT IN (SELECT value FROM json_each(?3))) UNION SELECT record_id FROM records WHERE space_id=?1 AND stream_id=?2 AND kind IN ('chat.message','file.shared') AND status IN ('LOCAL','ACCEPTED') AND record_id IN (SELECT value FROM json_each(?4))")?;
...
fn sources_for_ids(c: &Connection, ids: &[String]) -> Result<Vec<DisplaySource>> {
    let mut q=c.prepare("SELECT r.record_id,s.object_id,s.source_index, CASE
      WHEN EXISTS(SELECT 1 FROM outbox o WHERE o.record_id=r.record_id AND o.state='REJECTED') THEN 'REJECTED'
      WHEN EXISTS(SELECT 1 FROM outbox o WHERE o.record_id=r.record_id AND o.state='HELD_STALE_CONFIG') THEN 'HELD_STALE_CONFIG'
      WHEN EXISTS(SELECT 1 FROM replica_copies c JOIN outbox o USING(peer_id,mailbox_id,object_id) WHERE o.record_id=r.record_id AND c.missing=1) THEN 'REPAIR_PENDING'
