...
pub enum SyncError {
    ...
    Pruned(Box<replica::VerifiedPruned>),
...
impl Peer {
    ...
    pub(crate) fn with_delegated_access(mut self, proof: Option<String>) -> Self {
        self.delegated_access = proof;
        self
    }
    pub(crate) fn pairing_access(&self) -> Result<Option<String>> {
        self.signer
            .as_ref()
            .map(|signer| {
                signer
    ...
    pub async fn inventory(&self, after: u64) -> Result<Inventory> {
        ...
        if page.requested_messages.len() > 128 {
            return Err(SyncError::Peer);
        }
    ...
    pub async fn get(&self, id: ObjectId, size: u64) -> Result<Vec<u8>> {
        ...
        let response = self
            ...
            .await
            .map_err(network_error)?;
        let bytes = self.object_response(response, id, size as usize).await?;
    ...
    pub(crate) async fn post(
        ...
        self.post_with_status(id, bytes, hint, false)
            .await
    ...
    async fn post_with_status(
        ...
        let receipt = self
            .object_response(response, id, crate::record::MAX_RECORD)
            .await?;
    ...
    async fn object_response(
        ...
        if response.status().as_u16() == 410 {
            let bytes = bounded_bytes(response, crate::record::MAX_RECORD).await?;
            let proof = replica::VerifiedPruned::verify(&bytes, &self.key, self.mailbox(), object)
                .map_err(|_| SyncError::Transport(TransportFailure::InvalidReceiptResponse(410)))?;
            return Err(SyncError::Pruned(Box::new(proof)));
        }
...
async fn bounded_body(response: reqwest::Response, maximum: usize) -> Result<Vec<u8>> {
    ...
    bounded_bytes(response, maximum).await
}
async fn bounded_bytes(mut response: reqwest::Response, maximum: usize) -> Result<Vec<u8>> {
    if response
        .content_length()
...
pub struct SyncReport {
    ...
    pub quarantined: u64,
    pub retry: u64,
    /// Uploads refused because the target mailbox reached its storage quota.
...
impl SyncClient<'_> {
    ...
    async fn run(
            ...
            .filter(|p| p.descriptor.read_token.is_some())
        {
            ...
            self.store
                .resume_requested_copies(peer.id(), peer.mailbox(), page.requested_messages.clone())
                .await?;
            ...
            for entry in &page.entries {
                ...
                let bytes = if entry.transfer_hint == TransferHint::Lazy {
                    ...
                    match network(deadline, peer.get(entry.object_id, entry.size_bytes)).await {
                        ...
                        Err(SyncError::Pruned(proof)) => {
                            self.store.confirm_pruned(*proof).await?;
                            report.pruned += 1;
                            continue;
                        }
                        Err(SyncError::Peer) => {
                ...
                Some(ChatDecision::Accepted(verified)) => {
                    ...
                    if let Some((peer_id, mailbox, object, record, secret)) = acceptance
                        && let Some(peer) = self
                            .peers
                            .iter()
                            .find(|peer| peer.id() == peer_id && peer.mailbox() == mailbox)
                    {
    ...
    async fn upload_outbox(
        ...
        for _ in 0..limit {
            ...
            let message = opened
                .as_ref()
                .is_some_and(|r| r.body()["kind"] == "chat.message");
            ...
            let result = match peer {
                Some(p) => {
                    network_with_timeout(
                        ...
                        p.post_with_status(attempt.object_id(), bytes, hint, message),
                    )
                    .await
                }
