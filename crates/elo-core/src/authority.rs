...
impl Authority {
    ...
    pub fn direct_human_peer(&self, chat: &ChatMessage) -> Option<IdentityId> {
        let config = self.config(chat.config_id).ok()?;
        if config.chat_kind != Some(ChatKind::Direct) || chat.audience.len() != 2 {
            return None;
        }
        let humans = config
            .members
            .iter()
            .filter(|member| {
                member.identity_type == "HUMAN" && member.capabilities.contains(&Capability::Read)
            })
            .map(|member| member.identity_id)
            .collect::<Vec<_>>();
        (humans.len() == 2 && humans.iter().all(|id| chat.audience.contains(id)))
            .then(|| humans.into_iter().find(|id| *id != chat.issuer_identity))
            .flatten()
    }
}
