...
async fn discovery_backlog_resumes_without_poll_delay_and_delivers_a_new_dm_once() {
    ...
    let first = maya.operate(json!({"op":"invitation_sync"})).await.unwrap();
    ...
    for _ in 0..8 {
        ...
        let report = maya.operate(json!({"op":"invitation_sync"})).await.unwrap();
    ...
    maya.operate(json!({"op":"invitation_sync","force":true}))
        .await
        .unwrap();
    maya.operate(json!({"op":"sync_live"})).await.unwrap();
    assert_eq!(
