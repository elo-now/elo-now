...
// true when the system allows the app to display multiple scenes and the scene manifest permits multiple scenes
...
// UIKit adopts the scene lifecycle when a manifest is present, including
// single-window apps with UIApplicationSupportsMultipleScenes set to false.
pub unsafe fn scene_lifecycle_enabled() -> bool {
  NSBundle::mainBundle().infoDictionary().is_some_and(|info| {
    info.objectForKey(&NSString::from_str("UIApplicationSceneManifest")).is_some()
  })
}
  ...
  unsafe impl UISceneDelegate for TaoSceneDelegate {
    ...
    fn scene_willConnectToSession_options(
      ...
      unsafe {
        ...
        // Scene-based cold starts deliver links here, not through AppDelegate.
        // UIKit 27 can return nil for these collections on a plain launch,
        // despite the nonnull annotations in earlier SDK bindings.
        let urls: Option<Retained<NSSet<UIOpenURLContext>>> =
          msg_send![connection_options, URLContexts];
        if let Some(urls) = urls {
          self.scene_openURLContexts(objc2::sel!(scene:openURLContexts:), scene, &urls);
        }
        let activities: Option<Retained<NSSet<NSUserActivity>>> =
          msg_send![connection_options, userActivities];
        if let Some(activities) = activities {
          for activity in activities {
            self.scene_continueUserActivity(objc2::sel!(scene:continueUserActivity:), scene, &activity);
          }
        }
      }
    }
          ...
          .filter_map(|ctx| {
            ctx.URL().absoluteString().and_then(|url| {
              ...
              url
                .parse()
                .map_err(|e| {
                  log::error!("failed to parse scene URL: {e}");
                  e
                })
                .ok()
            })
    ...
    fn scene_continueUserActivity(&self, _scene: &UIScene, user_activity: &NSUserActivity) {
      unsafe {
        ...
        {
          if let Ok(url) = url.to_string().parse::<url::Url>() {
