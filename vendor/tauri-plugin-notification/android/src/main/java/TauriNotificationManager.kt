...
class TauriNotificationManager(
  ...
  fun handleNotificationActionPerformed(
    ...
    // Individually scheduled notices may have no serialized sourceJson.
    // The request ID carried by the PendingIntent remains valid after restart.
    val delivered = request ?: JSObject()
    delivered.put("id", notificationId)
    dataJson.put("notification", delivered)
    return dataJson
  }
