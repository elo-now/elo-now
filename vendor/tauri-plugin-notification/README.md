> Local patch of tauri-plugin-notification 2.4.0 (Apache-2.0 OR MIT).
> iOS reads delivered requests without assuming its process cache survived restart.
> iOS/Android retain the latest notification action until the app consumes it via
> `pending_action`, avoiding a lost tap before web-view listener registration.
> Android also returns the PendingIntent request ID when source JSON is absent,
